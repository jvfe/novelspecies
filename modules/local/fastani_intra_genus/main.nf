process FASTANI_INTRA_GENUS {
    ...
    script:
    def prefix = task.ext.prefix ?: "${meta.genus}_intra_genus"
    """
    fastANI \\
        --ql ${query_list} \\
        --rl ${query_list} \\
    ...
    awk -v comparison="intra_genus" 'BEGIN { OFS="\\t"; print "query","reference","ani_percent","comparison_type" }
    NF >= 3 {
        n = split(\$1, qp, "/"); q = qp[n]
